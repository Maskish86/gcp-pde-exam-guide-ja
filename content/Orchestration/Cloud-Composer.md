...
## 連携
- [[Storage/BigQuery|BigQuery]]: スケジュール済みクエリ、データセット保守、監視。
- [[Storage/Cloud-Storage|Cloud Storage]]: ファイル到着検知とステージングデータ。
- [[Processing/Dataflow|Dataflow]]: ストリーミング/バッチパイプラインの起動。
- [[Processing/Dataproc|Dataproc]]: 重い処理向けのSparkジョブ投入。
- [[Ingestion/PubSub|Pub/Sub]]: イベント駆動トリガーと通知。
- [[Security/Secret-Manager|Secret Manager]]: コネクション認証情報の保存・取得。
