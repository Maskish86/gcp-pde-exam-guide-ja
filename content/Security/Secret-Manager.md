...
## アクセス制御とIAM
**一般的なロール:**
- `roles/secretmanager.secretAccessor`: シークレットバージョンを読む。
- `roles/secretmanager.admin`: シークレットを管理する（控えめに使う）。

**設計のコツ:**
- サービスアカウントにアクセスを付与し、ユーザーには付与しない。
- 読み取り権と管理権を分離する。
- シークレットと環境（dev/prod）別に最小権限を使う。
